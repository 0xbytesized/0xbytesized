...
excerpt: "El incidente de seguridad de Vercel abril 2026: cómo un OAuth de terceros comprometió env vars no cifradas y qué implica para tu infraestructura."
---
...
- **Sensitive**: cifrada en reposo, irreducible una vez creada. Solo inyectable en Producción y Preview, no en Development.

El incidente reveló que muchos equipos marcan como "no sensibles" variables que **sí lo son** — API keys de bases de datos, tokens de servicio, credenciales de integraciones — porque el modo sensitive requiere eliminar y recrear la variable, y hasta ahora el interface no era tan evidente.
...
curl -s "https://admin.googleapis.com/admin/directory/v1.1/users/alice@ejemplo.com/tokens" \
  -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  | jq '.items[] | {clientId: .clientId, appName: .clientName, scopes: .scopes}'

# 3. Revocar una app sospechosa por su client ID
curl -X DELETE \
  "https://admin.googleapis.com/admin/directory/v1.1/users/alice@ejemplo.com/tokens/CLIENT_ID" \
  -H "Authorization: Bearer ${ACCESS_TOKEN}"
```
...
Este incidente es un ejemplo de textbook de defense in depth fallida. El atacante tuvo que superar varias capas, pero una vez dentro de la primera (el token OAuth), las siguientes capas no resistieron porque había variables sin cifrar. Para una infraestructura seria, considera:

- **Rotación automática de secrets**: herramientas como Doppler, Infisical o el propio Vercel con policies pueden forzar la rotación periódica.
...
- **Principle of least privilege para OAuth**: no conectes apps que no necesitas. Cada conexión es una puerta.
