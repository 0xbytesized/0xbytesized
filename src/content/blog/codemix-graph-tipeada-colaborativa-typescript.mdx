...
Si prefieres un encadenamiento fluido de tipo stream, `GraphTraversal` te da una API tipada — labels y propiedades se infieren en cada paso:
...
- **Hash** — O(1) para igualdad exacta, ideal para IDs y búsquedas directas.
...
Esto convierte el grafo en algo que puedes replicar, enviar y aplicar de forma distribuida. Me recuerda a los patrones de event sourcing, pero con la ergonomía de un grafo tipado.

## Mi veredicto

@codemix/graph cubre un hueco que nadie estaba llenando: una base de datos de grafos que puedes meter en tu aplicación de TypeScript, tipar con Zod, consultar con Cypher, recorrer con Gremlin, indexar correctamente y encima compartir en tiempo real entre pares sin nada de infraestructura. Es ambicioso para un proyecto que acaba de nacer (se publicó en npm el 8 de abril de 2026).

Lo que más valoro es la honestidad: está marcado como alpha, y el README advierte que lo usan en producción pero con cuidado. Me parece el enfoque correcto. La combinación de CRDT con grafos tipados es tan potente que merece la pena seguir el proyecto de cerca — nació hace apenas dos semanas en GitHub — pero todavía no es algo que pondría en producción sin una red de seguridad.
