...
Un emulador clásico es un bucle que decodifica una instrucción x86, modifica registros y memoria, y repite. Si le añades un JIT, el bucle se vuelve más rápido, pero introduces una barrera entre el código emulado y el nativo. Depurar un JIT es doloroso: las trazas de pila muestran código ensamblador generado dinámicamente, los puntos de interrupción del sistema no funcionan y el perfilado requiere herramientas específicas.
...
Veamos cómo se ve el modelo de memoria. En el runtime, `Memory` es un array de bytes plano sin paginación, compartido de forma insegura entre hilos porque el ejecutable objetivo ya controla todo el acceso:
