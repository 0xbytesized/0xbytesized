...
- **Neon (Databricks)** — bases de datos serverless Postgres
...
```typescript
// Antes: cada conversación era tierra quemida
...
- Aterrizar pull requests abiertas automáticamente
...
Codex puede generar e iterar sobre imágenes usando el modelo `gpt-image-1.5`. Combinando screenshots y código, sirve para crear mockups, conceptos visuales y assets de juegos sin salir del flujo de trabajo.

## Lo que me parece interesante

Hay tres cosas que destacan por encima del ruido:

**1. El computer use es pragmático, no una demo.** Cuando Anthropic mostró computer use, era impresionante pero se sentía experimental. Aquí está integrado en un producto que 3 millones de devs usan cada semana. La diferencia entre "mira lo que puede hacer" y "úso de esto para mi trabajo" es enorme.
...
**Jack of all trades, master of none.** Cuantas más cosas haga Codex, más se diluye. ¿Sigue siendo el mejor tool para escribir código, o se ha convertido en un Swiss Army knife donde ninguna hoja corta de verdad?
...
El computer use es el HEAD de la noticia, pero los plugins MCP y la memoria son el cambio que más impacto tendrá a largo plazo. Un agente que se integra con todo y aprende de ti es mucho más valioso que uno que puede hacer clic en botones.
